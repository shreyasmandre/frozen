...
int C2P(int N, double *p, double *c, double deta, double kx, double perm)
{
	...
	d[N-1] = tmp2 + (1e-6+fabs(kx))/deta; //  psin/deta**2 - psinm1/deta**2 + kx psin/deta = 0
    if(perm>1e5)
    {
        e[0] = 0; d[0] = 1;
    }
   ...
   double *concnew, const double *z, double dz, double kx, int N)
{
    ...
    d[0] = 1; du[0]= 0; concnew[0] = 0;
    d[N-1] = 1; dl[N-2] = -1+1e-6; concnew[N-1] = 0;
    {
...
double MatVec(const double *in, double *out, const double t1, const double t2, const double kx, double perm, const double *z, double dz, const int N)
{
    ...
    static int init=0;
    double t, dt, dtsuggest, derror, error=0, tolerance = 1e-7;
...
int main(int argc, char **argv)
{
    int N=401;
    int i,j;
    double L=100, dz, kx=2e-3, t1, t2, kx1, kx2;
    double res;
    double *z;
    double *p, *c;
    double perm=0.5;
