...
For autonomous systems, $\cA = \exp(\cL (t_2-t_1))$, and therefore $a(t_2,t_1) = \rho (\cA) = e^{\sigma (t_2-t_1)}$, where $\sigma$ is the eigenvalue of $\cL$ with the largest real part. Thus, $a(t_1,t_1)>1$ if and only if Re($\sigma$)$>0$, which is the classical criterion for linear instability.
Similarly, for time-periodic systems with period $T$ restricted to $t_2 -t_1 = n T$, $a(t_2, t_1) = \rho (\cA ) = e^{n \beta T}$, where $\beta$ is the Floquet exponent with the largest real part. Again $a(t_2,t_1)>1$ implies Re($\beta$)$>0$, which is the classical criterion for instability of periodic systems using Floquet theory.
